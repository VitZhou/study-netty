...
缓冲器是特定原始类型的元素的线性有限序列。 除了其内容(content)，缓冲区的基本属性是容量(capacity)，限制(limit)和位置(position)：
...
###	arrayOffset(): 返回缓冲区的第一个元素在支撑它的数组中的偏移量.(可选操作)
...
###	position(int newPosition): 设置这个缓冲区的位置。 如果mark被定义并且大于新位置，则它被丢弃。
...
###	clear(): 清除这个缓冲区。 position设置为零，limit设置为capacity，丢弃mark。
...
### hasRemaining(): 当前position是否到了限制Limit
